...
dotnet build Zhijian.slnx -c %CONFIGURATION% --no-restore
if errorlevel 1 goto :error

echo [3/3] Packing libraries...
for %%P in (
    "src\CodeWF.MindView\CodeWF.MindView.csproj"
    "src\CodeWF.MindView.Themes\CodeWF.MindView.Themes.csproj"
) do (
