...
function Test-ReleaseFile([System.IO.FileInfo]$File) {
    return $File.Extension -ine ".pdb" -and $File.Extension -ine ".zip"
}
...
$manifestPath = Join-Path $ArtifactsRoot "Zhijian-$releaseVersion-release-manifest.json"
...
foreach ($rid in $RuntimeIdentifier) {
    ...
    $publishDir = Join-Path $PublishRoot "$rid\Zhijian"
    if (-not (Test-Path -LiteralPath $publishDir -PathType Container)) {
        throw "Publish directory '$publishDir' was not found. Run publish.bat first or pass -PublishRoot."
    }
    ...
    $archiveName = "Zhijian-$releaseVersion-$rid.zip"
...
foreach ($plan in $plans) {
    ...
    Write-Host "Packaged $($plan.ArchiveName) (excluded $($plan.ExcludedFileCount) debug/archive files)"
}
